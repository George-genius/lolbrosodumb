...
public partial class MainWindow : Window
{
    ...
    private object ListPath(JsonElement args)
    {
        ...
        var entries = di.EnumerateFileSystemInfos().OrderBy(x => x is FileInfo ? 1 : 0).ThenBy(x => x.Name, StringComparer.OrdinalIgnoreCase).Select(ToFsEntry).ToList();
    ...
    private object DirectorySummary(JsonElement args)
    {
        ...
        foreach (var dir in Directory.EnumerateDirectories(path, "*", SearchOption.AllDirectories)) dirCount++;
        foreach (var file in Directory.EnumerateFiles(path, "*", SearchOption.AllDirectories)) { fileCount++; totalSize += new FileInfo(file).Length; }
    ...
    private object LargestFiles(JsonElement args)
    {
        ...
        if (!Directory.Exists(path)) throw new DirectoryNotFoundException($"Directory not found: {path}");
        var items = Directory.EnumerateFiles(path, "*", SearchOption.AllDirectories).Select(p => new FileInfo(p)).OrderByDescending(fi => fi.Length).Take(limit).Select(fi => new { path = NormalizeForUi(fi.FullName), name = fi.Name, size = fi.Length }).ToList();
    ...
    private static int GetInt(JsonElement args, string name, int fallback)
    {
        ...
        return value.ValueKind switch
        {
            ...
            JsonValueKind.String when int.TryParse(value.GetString(), out parsed) => parsed,
            _ => fallback
        };
    }
